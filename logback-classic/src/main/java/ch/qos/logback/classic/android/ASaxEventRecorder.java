...
public class ASaxEventRecorder extends SaxEventRecorder {
    ...
    private void startElement(XmlPullParser xpp) {
        ...
        if (filter.checkStart(name)) {
            ...
            for (int i = 0; i < xpp.getAttributeCount(); i++) {
                atts.addAttribute(xpp.getAttributeNamespace(i), xpp.getAttributeName(i), xpp.getAttributeName(i), xpp.getAttributeType(i),
    ...
    private void checkForWatchedAttributes(XmlPullParser xpp) {
        if (elemNameToWatch != null && elemAttrs == null && xpp.getName().equals(elemNameToWatch)) {
